        ...
        if not model:
            ...
            if not available_server_list:
                ...
                if server_status == "running":
                        ...
                        try:
                            ...
                            if not response:
                                ...
                                celery_log_prompt_response.delay(
                                    is_session_start_node=None, key_object_id=key_object.id, llm_id=model.id, prompt=data.prompt, response=response, type_='agent_api')
                                return 200, {
                                    'context': context,
                    ...
                    command_EC2.delay(inference_server.name,
                                      region=constant.REGION, action="on")
