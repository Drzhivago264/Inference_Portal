	...
	websocket.current.onmessage = (message) => {
		...
		if (dataFromServer) {
				...
				if (!use_user_template) {
					...
					if (editorref) {
						editorref.current.render(
							JSON.parse(dataFromServer.swap_template)
						);
					}
				} else {
					const newChildTemplateList =
			...
			if (
				...
				dataFromServer.agent_action &&
				(dataFromServer.agent_action === "STOP" ||
					dataFromServer.agent_action === "NEXT")
			) {
				const blockToAdd = {
					type: "paragraph",
					data: {
				...
				setChatMessage((chat_message) => [
					...chat_message.slice(0, -1),
					{
						...
						holderid:
							chat_message[chat_message.length - 1].holderid,
			...
			let logTa =
				...
				document.getElementById("chat-log-agent");
