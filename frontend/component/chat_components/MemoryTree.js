...
export const MemoryTree = () => {
	...
	const { root_node, default_expand_node, total_node, refetch } =
		useGetMemoryTree(page_num);
				...
				{data.map((parent) => {
					return (
						<TreeItem
							...
							{parent.children && (
								<RecursiveMemoryTree data={parent.children} />
							)}
						</TreeItem>
					);
				})}
	...
	return (
		<Paper variant='outlined'>
			...
			{root_node && default_expand_node && (
				<RecursiveMemoryTree data={root_node} />
			)}
			{!root_node && (
				<Typography variant='body2'>There is no memory yet.</Typography>
			)}
