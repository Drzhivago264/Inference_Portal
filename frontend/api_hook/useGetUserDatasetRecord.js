...
export const useGetUserDatasetRecord = (
	setDatasetIsLoading,
	setRecordList,
	dataset_list,
	dataset_row,
	...
	const {error, isLoading, refetch} = useQuery(
		...
		() => baseGet(`/frontend-api/get-dataset-record/${dataset_list[selectedIndex].id}?page=${paginationModel.page + 1}`),
		{
			...
			onSuccess: (data) => {
				...
				const column = [{field: "system_prompt", headerName: "System Prompt", width: 200, editable: false, disableColumnMenu: true}];
				const column_name = ["system_prompt"];
				const row = [];
				for (let key in dataset_list[selectedIndex].default_content_structure) {
					let new_column = {
						field: dataset_list[selectedIndex].default_content_structure[key]["unique"],
						headerName: dataset_list[selectedIndex].default_content_structure[key]["name"],
						width: 200,
						editable: false,
				...
				for (let eva in dataset_list[selectedIndex].default_evaluation) {
					let new_column = {
						field: dataset_list[selectedIndex].default_evaluation[eva]["unique"],
						headerName: dataset_list[selectedIndex].default_evaluation[eva]["evaluation_name"],
						width: 100,
						editable: false,
						disableColumnMenu: true,
					};
