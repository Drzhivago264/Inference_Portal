...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if  key_object == False:
            ...
            Inference.delay(unique=unique_response_id,mode = mode, type_ = "chatroom", stream = True, key=self.key, key_name = self.name, credit = key_object.credit, room_group_name = self.room_group_name, model = choosen_models, top_k=top_k, top_p =top_p, best_of =best_of, temperature =temperature, max_tokens = max_tokens, presense_penalty =presense_penalty, frequency_penalty = frequency_penalty, length_penalty = length_penalty, early_stopping = early_stopping,beam = beam, prompt=message)
    ...
    async def receive(self, text_data):
        ...
        if 'paragraph' in text_data_json:
            ...
            if  key_object == False:
                ...
                unique_response_id = str(uuid.uuid4())
                text = message
                #response = self.agent.chat(message=text, model=choosen_models, room_group_name = self.room_group_name, credit=key_object.credit, unique=unique_response_id)
                #print(response)
                # Send message to room group
                await self.channel_layer.group_send(
                        self.room_group_name, {"type": "chat_message", 
