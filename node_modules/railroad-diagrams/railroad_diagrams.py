...
class Choice(DiagramItem):
    ...
    def format(self, x, y, width):
        ...
        for i, item in list(enumerate(above))[::-1]:
            ...
            Path(x + ARC_RADIUS * 2 + innerWidth, y - distanceFromY).arc("ne").down(
                distanceFromY - ARC_RADIUS * 2
            ).arc("ws").addTo(self)
            distanceFromY += max(
                ARC_RADIUS,
                item.up
        ...
        for i, item in enumerate(below):
            if i == 0:
                distanceFromY = max(
                    ARC_RADIUS * 2,
                    self.items[self.default].down + VERTICAL_SEPARATION + item.up,
                )
            Path(x, y).arc("ne").down(distanceFromY - ARC_RADIUS * 2).arc("ws").addTo(
                self
            )
            item.format(x + ARC_RADIUS * 2, y + distanceFromY, innerWidth).addTo(self)
            Path(x + ARC_RADIUS * 2 + innerWidth, y + distanceFromY).arc("se").up(
                distanceFromY - ARC_RADIUS * 2
            ).arc("wn").addTo(self)
            distanceFromY += max(
                ARC_RADIUS,
...
class OneOrMore(DiagramItem):
    ...
    def format(self, x, y, width):
        ...
        # Draw repeat arc
        distanceFromY = max(
            ARC_RADIUS * 2, self.item.down + VERTICAL_SEPARATION + self.rep.up
        )
        ...
        Path(x + self.width - ARC_RADIUS, y + distanceFromY).arc("se").up(
            distanceFromY - ARC_RADIUS * 2
        ).arc("en").addTo(self)
...
if __name__ == "__main__":
    ...
    sys.stdout.write(
        "<!doctype html><title>Test</title><style>svg.railroad-diagram{background-color:hsl(30,20%,95%);}svg.railroad-diagram path{stroke-width:3;stroke:black;fill:rgba(0,0,0,0);}svg.railroad-diagram text{font:bold 14px monospace;text-anchor:middle;}svg.railroad-diagram text.label{text-anchor:start;}svg.railroad-diagram text.comment{font:italic 12px monospace;}svg.railroad-diagram rect{stroke-width:3;stroke:black;fill:hsl(120,100%,90%);}</style>"
    )
