...
class LLMAdmin(admin.ModelAdmin):
    model = LLM
    list_display = ('name', 'desc',  'input_price', 'output_price', 'agent_availability', 'is_self_host', 'context_length', 'max_history_length')
    list_editable = ( 'desc', 'input_price', 'output_price', 'agent_availability', 'is_self_host', 'context_length', 'max_history_length')
    list_display_links = ('name',)

@admin.register(Dataset)
class DatasetAdmin(admin.ModelAdmin):
    model = Dataset
    list_display = ('name', 'user', 'default_system_prompt', 'default_evaluation', 'created_at', 'updated_at')
...
class DatasetRecordAdmin(admin.ModelAdmin):
    model = DatasetRecord
    list_display = ('dataset', 'system_prompt', 'prompt', 'response', 'evaluation', 'created_at', 'updated_at')
...
class InferenceServerAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'instance_type', 'url', 'alternative_url', 'hosted_model', 'public_ip', 'private_ip', 'status', 'last_message_time', 'availability')
    list_editable = ('instance_type', 'url', 'alternative_url', 'public_ip', 'private_ip', 'status', 'last_message_time', 'availability')
    list_display_links = ('name', 'hosted_model')
    
@admin.register(PromptResponse)
class PromptResponseAdmin(admin.ModelAdmin):
    model = PromptResponse
    list_display = ('prompt', 'response', 'type' ,'model', 'key' ,'input_cost', 'output_cost', 'number_input_tokens', 'number_output_tokens')
    list_display_links = ('model', 'key')

@admin.register(PaymentHistory)
class PaymentHistoryAdmin(admin.ModelAdmin):
    model = PaymentHistory
    list_display = ('key', 'crypto', 'amount', 'integrated_address', 'payment_id', 'transaction_hash', 'locked', 'block_height')
    list_editable = ('amount', 'integrated_address', 'payment_id', 'transaction_hash', 'locked', 'block_height')
