            ...
            if record_count == 0:
                ...
                """unique = uuid.uuid4().hex
                    task = export_large_dataset.delay(
                        dataset_id=dataset_id, url_safe_datasetname=url_safe_datasetname, unique=unique, extension=extension
                    )
                return Response(
                    {
                        "download_link": f"https://dataset.professorparakeet.com/{url_safe_datasetname}_{unique}{extension}",
                        "task_id": task.id,
                        "id": dataset.id,
                        "name": dataset.name,
                        "export_type": "celery"
                ...
                return Response(
                    {"detail": "I am working on this feature, wait!"},
