...
class CreditBalanceApiTestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            self.key_object.hashed_key, "", self.key_object.hashed_key
        )
    ...
    def test_credit_balance_api(self):
        ...
        response = self.client.post(
            "/frontend-api/check-credit", data={"key": self.key, "key_name": self.name}
        )
    ...
    def test_credit_balance_api_wrong_credential(self):
        ...
        response = self.client.post(
            ...
            data={"key": "test_failed", "key_name": "test_failed"},
        )
        self.assertEqual(response.status_code, 403)
        self.assertEqual(
            response.json(), {"detail": "Your Key Name and/or Key is/are incorrect"}
        )
