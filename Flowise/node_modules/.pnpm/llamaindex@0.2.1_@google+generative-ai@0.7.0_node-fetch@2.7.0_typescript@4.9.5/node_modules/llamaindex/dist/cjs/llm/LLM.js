...
_export(exports, {
    ...
    ALL_AVAILABLE_LLAMADEUCE_MODELS: function() {
        return ALL_AVAILABLE_LLAMADEUCE_MODELS;
    },
...
const _open_ai = require("./open_ai.js");
const _portkey = require("./portkey.js");
const _replicate_ai = require("./replicate_ai.js");
const GPT4_MODELS = {
    "gpt-4": {
        contextWindow: 8192
    },
...
const isFunctionCallingModel = (model)=>{
...
class OpenAI extends _base.BaseLLM {
    ...
    constructor(init){
        ...
        if (init?.azure || (0, _azure.shouldUseAzure)()) {
            ...
            this.session = init?.session ?? (0, _open_ai.getOpenAISession)({
                azure: true,
                apiKey: this.apiKey,
    ...
    tokens(messages) {
        ...
        const tokenizer = _GlobalsHelper.globalsHelper.tokenizer(this.metadata.tokenizer);
    ...
    async chat(params) {
        ...
        const baseRequestParams = {
            ...
            messages: this.toOpenAIMessage(messages),
            top_p: this.topP,
    ...
    async *streamChat({ messages, parentEvent }) {
        ...
        const chunk_stream = await this.session.openai.chat.completions.create({
            ...baseRequestParams,
            stream: true
        });
...
class LlamaDeuce extends _base.BaseLLM {
    model;
    chatStrategy;
    temperature;
    topP;
    ...
    constructor(init){
        ...
        this.chatStrategy = init?.chatStrategy ?? (this.model.endsWith("4bit") ? "replicate4bitwnewlines" : "metawbos"); // With BOS and EOS seems to work best, although they all have problems past a certain point
        this.temperature = init?.temperature ?? 0.1; // minimum temperature is 0.01 for Replicate endpoint
        this.topP = init?.topP ?? 1;
        this.maxTokens = init?.maxTokens ?? ALL_AVAILABLE_LLAMADEUCE_MODELS[this.model].contextWindow; // For Replicate, the default is 500 tokens which is too low.
        this.replicateSession = init?.replicateSession ?? new _replicate_ai.ReplicateSession();
    }
    tokens(messages) {
...
class Anthropic extends _base.BaseLLM {
    ...
    constructor(init){
        ...
        this.maxRetries = init?.maxRetries ?? 10;
        this.timeout = init?.timeout ?? 60 * 1000; // Default is 60 seconds
        this.session = init?.session ?? (0, _anthropic.getAnthropicSession)({
            apiKey: this.apiKey,
            maxRetries: this.maxRetries,
            timeout: this.timeout
        });
    ...
    async *streamChat(messages, parentEvent, systemPrompt) {
...
class Portkey extends _base.BaseLLM {
    apiKey = undefined;
    baseURL = undefined;
    mode = undefined;
    llms = undefined;
    ...
    constructor(init){
        ...
        this.mode = init?.mode;
        this.llms = init?.llms;
        this.session = (0, _portkey.getPortkeySession)({
            apiKey: this.apiKey,
    ...
    async *streamChat(messages, parentEvent, params) {
