...
Object.defineProperty(exports, "ChromaVectorStore", {
    enumerable: true,
    get: function() {
        return ChromaVectorStore;
    }
});
const _chromadb = require("chromadb");
...
class ChromaVectorStore {
    ...
    constructor(init){
        ...
        this.chromaClient = new _chromadb.ChromaClient(init.chromaClientParams);
    ...
    getDataToInsert(nodes) {
        const metadatas = nodes.map((node)=>(0, _utils.nodeToMetadata)(node, true, this.textKey, this.flatMetadata));
        return {
            embeddings: nodes.map((node)=>node.getEmbedding()),
            ids: nodes.map((node)=>node.id_),
            metadatas,
            documents: nodes.map((node)=>node.getContent(_Node.MetadataMode.NONE))
        };
    }
    async add(nodes) {
    ...
    async query(query, options) {
        ...
        if (query.mode != _types.VectorStoreQueryMode.DEFAULT) {
        ...
        const queryResponse = await collection.query({
            ...
            include: [
                _chromadb.IncludeEnum.Distances,
                _chromadb.IncludeEnum.Metadatas,
                _chromadb.IncludeEnum.Documents,
                _chromadb.IncludeEnum.Embeddings
            ]
        });
