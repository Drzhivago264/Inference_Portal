...
import { AgentChatResponse, ChatResponseMode, StreamingAgentChatResponse } from "../../engines/chat/types.js";
 ...
 */ export class OpenAIAgentWorker {
   ...
   */ _processMessage(task, chatResponse) {
    ...
    async _getStreamAiResponse(task, llmChatKwargs) {
        ...
        const iterator = streamConverter(streamReducer({
            stream,
            initialValue: "",
   ...
   */ async getTools(input) {
    ...
    async _runStep(step, task, mode = ChatResponseMode.WAIT, toolChoice = "auto") {
        const tools = await this.getTools(task.input);
        if (step.input) {
            addUserStepToMemory(step, task.extraState.newMemory, this.verbose);
        }
        const openaiTools = tools.map((tool)=>toOpenAiTool({
